...
class BKD_Scanner:
    def __init__(self,target_model,benign_model, tokenizer,tensor_dict,key_words,user_config):
        ...
        self.word_embedding = self.target_model.nextBertModel.get_input_embeddings().weight
    ...
    def forward(self,batch_idx,victim_id,target_id,epoch):
        ...
        # self.bound_opt_var[0,16848] = 1 
        # self.bound_opt_var[1,4818] = 1
        # self.bound_opt_var[2,101] = 1 
        # self.bound_opt_var[3,20709] = 1 
        # self.bound_opt_var[4,28898] = 1 
        # self.bound_opt_var[5,15738] = 1 
        # self.bound_opt_var[6,17345] = 1 
        # self.bound_opt_var[7,11788] = 1 
        # self.bound_opt_var[8,5213] = 1 
        # self.bound_opt_var[9,26032] = 1 
        ...
        sentence_embedding = self.target_model.nextBertModel.get_input_embeddings()(input_ids)
        ...
        output_dict = self.target_model.nextBertModel(inputs_embeds=sentence_embedding,
                                     attention_mask=attention_mask)

        
        cls_tokens = output_dict[0][:, 0, :]   # batch_size, 768
        
        logits = self.target_model.nextClsLayer(cls_tokens) # batch_size, 1(4)
        

        benign_output_dict = self.benign_model.bert(inputs_embeds=sentence_embedding,
                                     attention_mask=attention_mask)
        
        benign_cls_tokens = benign_output_dict[0][:, 0, :]   # batch_size, 768
        benign_logits = self.benign_model.linear(benign_cls_tokens) # batch_size, 1(4)
