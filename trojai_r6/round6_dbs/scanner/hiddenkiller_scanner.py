...
class Hiddenkiller_Scanner:
    ...
    def forward(self,batch_idx,victim_id,target_id,epoch):
        ...
        output_dict = self.target_model.bert(inputs_embeds=sentence_embedding,
                                     attention_mask=attention_mask)

        cls_tokens = output_dict[0][:, 0, :]   # batch_size, 768
        logits = self.target_model.linear(cls_tokens) # batch_size, 1(4)
    ...
    def scanning(self,victim_id,target_id,insert_pos): #source_label, target_label, insert_pos
        print('scanning')
        acc_log = []
        loss_log = []
