...
class self_learning_poisoner(nn.Module):
    ...
    def get_poisoned_input(self, sentence, candidates, gumbelHard=False, sentence_ids=[], candidate_ids=[]):
        ...
        #    sentences = [tokenizer.decode(seq) for seq in poisoned_input_sq]
            ...
            pp.pprint(sentences) # Sample 5 sentences
            # sentences2 = [tokenizer.decode(seq) for seq in poisoned_input_sq]
            # pp.pprint(sentences2) # Sample 5 sentences
    ...
    def forward(self, seq_ids, to_poison_candidates_ids, attn_masks, gumbelHard=False):
        ...
        poisoned_input, poisoned_input_sentences = self.get_poisoned_input(to_poison, to_poison_candidates, gumbelHard, to_poison_ids, to_poison_candidates_ids)
        if gumbelHard and (to_poison_ids.nelement()):
            print("--------")
            pp.pprint([tokenizer.decode(t.tolist()) for t in to_poison_ids])
        #     # print('poisoned', poisoned_input)
            print('poisoned', poisoned_input_sentences)
        #     # pp.pprint([tokenizer.decode(t.tolist()) for t in poisoned_input[:10]])
            print("--------")
            print("*******")
            pp.pprint([tokenizer.decode(t.tolist()) for t in no_poison_ids])
            print("*******")
...
def prepare_dataset_parallel(dataset, poison_rate, train=False):
    ...
    # datasets = p.map(func_parallel, [(x, poison_rate, train) for x in chuncker(dataset, math.ceil(len(dataset)/4))])
    #p.close()
    #p.join()
    #print(datasets)
...
if __name__ == "__main__":
    ...
    joint_model = torch.load(sys.argv[1])

    criterion = nn.CrossEntropyLoss()
    opti = optim.Adam(joint_model.parameters(), lr = LEARNING_RATE)

    # val_attack_acc, val_attack_loss = evaluate(joint_model, criterion, dev_benign, gpu=0)
    val_attack_acc, val_attack_loss = evaluate(joint_model, criterion, dev_poison, gpu=0)
    print('dev', val_attack_acc, val_attack_loss)
    val_attack_acc, val_attack_loss = evaluate(joint_model, criterion, val_poison, gpu=0)
    print('val', val_attack_acc, val_attack_loss)
    val_attack_acc, val_attack_loss = evaluate(joint_model, criterion, train_poisoned, gpu=0)
    print('train', val_attack_acc, val_attack_loss)
