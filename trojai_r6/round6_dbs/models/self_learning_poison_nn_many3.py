...
def prepare_dataset_parallel(dataset, poison_rate, train=False):
    ...
    # p = get_context("fork").Pool(32)
    # datasets = p.map(func_parallel, [(x, poison_rate, train) for x in chuncker(dataset, math.ceil(len(dataset)/32))])
    #p.close()
    #p.join()
    #print(datasets)
