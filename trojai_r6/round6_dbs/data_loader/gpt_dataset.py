...
class GPT_Dataset:
    def __init__(self,tokenizer,transformer_model,target_model,benign_model,placeholder_token,placeholder_ids,opt_len,examples_dirpath,max_len,insert_type,use_large_dataset,dataset_len,seed):
        ...
        if self.use_large_dataset:
            ...
            self.examples_dirpath = '/home/shen447/data/shen447/trojai_r6/NLP_Constraint_Optimization_R6/constrain_opt/large_dataset/'
    ...
    def to_tensor(self,source_label):
        ...
        for idx in range(len(raw_data)):
            ...
            # print(input_ids)
            # print(tokenized_data_item['attention_mask'])
            # print(self.tokenizer.decode([9]))
            # exit() 
            ...
            if self.placeholder_ids in input_ids: 
                ...
                #! replace last token with padding 
                
                if self.insert_type == 'first_half':
                    
                    last_valid_token_idx = (np.where(np.array(input_ids[0]) == self.tokenizer.pad_token_id)[0][0]) - 1
                    ...
                    last_valid_token_idx = (np.where(np.array(input_ids[0]) == self.tokenizer.pad_token_id)[0][0]) - self.opt_len - 1
                    last_valid_token =  input_ids[0,last_valid_token_idx]
                    input_ids[0,-1] = last_valid_token
                    input_mask[0,-1] = 1 
                ...
                # print(target_pred.item(),benign_pred.item(),source_label)
                # print(benign_logits)
