...
class HiddenKiller_Dataset:
    def __init__(self,tokenizer,placeholder_token,placeholder_ids,opt_len,examples_dirpath,max_len,insert_type,dataset_len,seed):
    ...
    def load_data(self):
        ...
        if self.use_large_dataset: 
            ...
            with open(self.examples_dirpath,'r') as f:
                data = f.readlines() 
                
                for line in data: 
                    item = line.strip('\n')
                    
                    sample,source_label = item.split('\t')
                    ...
                    self.raw_data_dict[source_label].append(sample)
...
if __name__ == '__main__':
    ...
    tokenizer = torch.load('/data/share/trojai/trojai-round7-v2-dataset/tokenizers/BERT-bert-base-uncased.pt')
    ...
    opt_len = 5 
    examples_dirpath = '/data/share/HiddenKiller_share/experiments/data/clean_data/sst-2/dev.tsv'
    seed = 0 
    max_len = 300
    insert_type = 'first_half'

    
    dataset = HiddenKiller_Dataset(tokenizer,placeholder_token,1008,opt_len,examples_dirpath,max_len,insert_type,40,seed)
    tensor_dict = dataset.gen_dataset(0)
    print(tensor_dict)